...
def infer(
    ...
    **predict_kwargs: Dict[str, Any],
    ...
    if log_result_set:
        ...
        artifact_path = context.artifact_path + f'/{batch_id}/'
        context.log_dataset(
        key=result_set_name,
            df=result_set,
            tag=artifacts_tag,
    ...
    if perform_drift_analysis:
        ...
        context.log_artifact(drift_table_plot, tag=artifacts_tag,artifact_path=artifact_path)
        context.log_artifact(metric_per_feature_dict, tag=artifacts_tag,artifact_path=artifact_path)
        context.log_results(results=analysis_results)
        context.set_label(key='batch_id',value=batch_id)
